...
public final class ItemSwipeHelperInteractorTest extends BaseInstrumentedTest {
	...
	public void testGetMovementFlags() {
		...
		when(mockAdapter.getItemSwipeFlags(RecyclerView.NO_POSITION)).thenReturn(ItemSwipeHelper.makeSwipeFlags(ItemSwipeHelper.START | ItemSwipeHelper.END));
		assertThat(
				interactor.getMovementFlags(mMockRecyclerView, mock(Holder.class)),
				is(ItemSwipeHelper.makeSwipeFlags(ItemSwipeHelper.START | ItemSwipeHelper.END))
		);
		verify(mockAdapter, times(1)).getItemSwipeFlags(RecyclerView.NO_POSITION);
	}
	...
	public void testOnSelectedChanged() throws Exception {
		...
		final Adapter mockAdapter = mock(Adapter.class);
		final Holder mockHolder = createMockHolder(view);
	...
	public void testClearView() throws Exception {
		final View view = new View(mContext);
		final Adapter mockAdapter = mock(Adapter.class);
		final Holder mockHolder = createMockHolder(mMockRecyclerView, view);
		final ItemSwipeHelper.OnSwipeListener mockListener = mock(ItemSwipeHelper.OnSwipeListener.class);
		final ItemSwipeHelper helper = new ItemSwipeHelper();
		final ItemSwipeHelper.Interactor interactor = new ItemSwipeHelper.Interactor();
		interactor.attachToHelper(helper);
